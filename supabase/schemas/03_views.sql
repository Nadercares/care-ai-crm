...
-- Pattern analytics (Phase 12)
create or replace view public.carrier_patterns_summary
with (security_invoker = on)
as
select
    c.id as carrier_id, c.name as carrier_name, c.naic_code, c.default_state,
    ...
    count(s.id) filter (where s.method in ('appraisal','mediation','litigation')) as count_escalated,
...
as
select
    ca.id as carrier_adjuster_id, ca.first_name, ca.last_name,
    coalesce(nullif(trim(ca.first_name || ' ' || ca.last_name), ''), 'Unnamed') as adjuster_name,
    ca.license_number, ca.license_state, ca.role, ca.email, ca.phone, ca.carrier_id,
    car.name as carrier_name,
    count(distinct cl.id) as claim_count,
    count(distinct cl.id) filter (where s.id is not null) as settled_count,
    coalesce(sum(s.settlement_amount), 0)::numeric(14,2) as total_settled_amount,
...
left join public.settlements s on s.claim_id = cl.id
group by ca.id, ca.first_name, ca.last_name, ca.license_number, ca.license_state, ca.role, ca.email, ca.phone, ca.carrier_id, car.name;
